const insertionSort = (arr, comparatorFun) => {
  if(typeof comparatorFun !== "function") comparatorFun = (a, b) => a - b;

  for (let i = 0; i < arr.length; i++) {
    let newValueToSort = arr[i];
    for (let j = i - 1; j >= 0; j--) {
      if(comparatorFun(newValueToSort, arr[j]) < 0){
        arr[j + 1] = arr[j];
        if(j === 0) arr[j] = newValueToSort;
      }else{
        arr[j + 1] = newValueToSort;
        break;
      }
    }
  }
...
const arrOfNumbers = [4, 3, 5, 3, 43, 232, 4, 34, 232, 32, 4, 35, 34, 23, 2, 453, 546, 75, 67, 4342, 32];
console.log(insertionSort(arrOfNumbers)); 
